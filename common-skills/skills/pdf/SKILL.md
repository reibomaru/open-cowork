...
name: pdf
description: >
  PDF の読み取り・生成・変換・フォーム入力を行うスキル。成果物または入力が PDF のときに使う。
  テキスト/表抽出、ページ画像化、結合・分割・回転、フォーム入力、HTML 帳票や Office ドキュメントの PDF 化をカバー。
  トリガー例: 「この PDF から本文を抜き出して」「請求書を PDF で作って」「資料を PDF に変換して」
  「PDF をページごとに画像化して」「PDF フォームに入力して」「複数 PDF を結合して」「.pdf を読んで要約して」など、
